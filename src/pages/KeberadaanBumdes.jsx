...
// Spreadsheet: 1507_Data BUMDES dan BUMDESMA per 30 JUNI 2024
// https://docs.google.com/spreadsheets/d/1E7OAeKCQF7vRJZDVw8oxBq0RzFbNLZTkZK6-r3NoCmM/edit?gid=1142183420
const SPREADSHEET_ID = "1E7OAeKCQF7vRJZDVw8oxBq0RzFbNLZTkZK6-r3NoCmM";
const GID_BUMDES = "1142183420";
const CSV_BUMDES = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/export?format=csv&gid=${GID_BUMDES}`;

// URL Web App Apps Script yang menangani update kolom D dan E
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzMkYli42srdvawhdtgylvRuskmAIErv1Q9wPRYbyrdli0hJ8xovyajf7GAaa2nkQ/exec";
...
// Opsi hasil identifikasi (sesuai dropdown kolom D di spreadsheet)
const OPSI_HASIL_IDENTIFIKASI = [
  { value: "sudah di data", label: "Sudah di Data", color: "emerald" },
  { value: "belum di data", label: "Belum di Data", color: "rose" },
];
...
// ── Helpers warna card kecamatan (berdasarkan jumlah belum didata) ──
function countBadge(v) {
  if (v === 0) return "bg-gray-50 text-gray-500 ring-gray-200";
  if (v <= 3) return "bg-blue-50 text-blue-700 ring-blue-200";
  if (v <= 6) return "bg-amber-50 text-amber-700 ring-amber-200";
...
function hasilBadge(v) {
  const s = (v || "").trim().toLowerCase();
  if (s === "sudah di data") return "bg-emerald-50 text-emerald-700 ring-emerald-200";
  if (s === "belum di data") return "bg-rose-50 text-rose-700 ring-rose-200";
...
// ── Card kecamatan ──
// Angka besar = jumlah BELUM didata, sub-info = jumlah sudah didata
...
// ── Baris Badan Usaha Milik Desa ──
function BumdesRow({ row, rank, onDetail }) {
  return (
    <div className="py-3 border-b border-gray-50 last:border-0">
      <div className="flex items-start gap-3">
        <span className="text-xs font-bold text-gray-300 w-5 text-right flex-shrink-0 mt-0.5">{rank}</span>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold text-gray-700 truncate">{row.namaBumDesa}</p>
          <p className="text-xs text-gray-400 truncate mt-0.5">Desa {row.desa || "-"}</p>
          <div className="mt-1.5 space-y-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] text-gray-400 font-medium w-[105px] flex-shrink-0">Hasil Identifikasi</span>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ring-1 whitespace-nowrap ${hasilBadge(row.hasilIdentifikasi)}`}>
                {row.hasilIdentifikasi ? row.hasilIdentifikasi : "Belum Diketahui"}
              </span>
            </div>
            {row.keteranganLainnya && (
              <div className="flex items-start gap-1.5 flex-wrap">
                <span className="text-[10px] text-gray-400 font-medium w-[105px] flex-shrink-0">Keterangan</span>
                <span className="text-[10px] text-gray-500">{row.keteranganLainnya}</span>
              </div>
            )}
          </div>
        </div>
...
function DetailBumdes({ row, onClose, onSaved }) {
  const [hasilIdentifikasi, setHasilIdentifikasi] = useState(row.hasilIdentifikasi || "");
  const [keteranganLainnya, setKeteranganLainnya] = useState(row.keteranganLainnya || "");
  ...
  const handleSave = async () => {
    ...
    if (!hasilIdentifikasi) {
      setErrorMsg("Pilih hasil identifikasi terlebih dahulu.");
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={isBusy ? undefined : onClose}>
      ...
      >
        ...
        <div className="px-5 py-4" style={{ background: "linear-gradient(135deg,#F5A623 0%,#e8820a 100%)" }}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-orange-100 text-[10px] font-semibold uppercase tracking-widest mb-0.5">Konfirmasi Data Badan Usaha Milik Desa</p>
              <h2 className="text-white text-base sm:text-lg font-black break-words leading-tight">{row.namaBumDesa}</h2>
            </div>
        ...
        <div className="px-5 py-4 space-y-3.5 max-h-[70vh] overflow-y-auto">
          <table className="w-full text-xs border border-gray-100 rounded-xl overflow-hidden">
            <tbody>
              <tr className="border-b border-gray-100">
                <td className="bg-gray-50 px-3 py-2 font-semibold text-gray-500 w-1/3 align-top">Kecamatan</td>
          ...
          <div className="pt-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              {OPSI_HASIL_IDENTIFIKASI.map(o => {
                const isSel = hasilIdentifikasi === o.value;
                const styles = o.color === "emerald"
                  ? (isSel ? "border-emerald-400 bg-emerald-50 text-emerald-700 shadow-sm shadow-emerald-100" : "border-gray-200 bg-white text-gray-500 hover:border-emerald-200")
                  : (isSel ? "border-rose-400 bg-rose-50 text-rose-700 shadow-sm shadow-rose-100" : "border-gray-200 bg-white text-gray-500 hover:border-rose-200");
                return (
                  <button
                    key={o.value}
                    type="button"
                    disabled={isBusy}
                    onClick={() => setHasilIdentifikasi(o.value)}
                    className={`px-3 py-2.5 rounded-xl border-2 text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${styles}`}
                  >
                    {o.label}
                  </button>
                );
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex gap-2 justify-end bg-gray-50">
          ...
          <button
            onClick={handleSave}
            disabled={isBusy || !hasilIdentifikasi}
...
// KOMPONEN UTAMA
// ══════════════════════════════════════════════════════════════════════════════
export default function KeberadaanBumdes() {
  ...
  const [searchBumdes, setSearchBumdes] = useState("");
  ...
  // ── Fetch CSV ──
  useEffect(() => {
    fetch(`${CSV_BUMDES}&_cb=${Date.now()}`)
      .then(r => { if (!r.ok) throw new Error("Gagal mengambil data Badan Usaha Milik Desa."); return r.text(); })
      .then(text => {
        ...
        parsed.slice(1).forEach((cols, idx) => {
          ...
          const kecamatan = (cols[0] || "").trim();
          const namaBumDesa = (cols[2] || "").trim();
          if (!namaBumDesa) return;
          ...
          data.push({
            ...
            desa: (cols[1] || "").trim(),
            namaBumDesa,
            hasilIdentifikasi: (cols[3] || "").trim(), // D
            keteranganLainnya: (cols[4] || "").trim(), // E
          });
        });
  ...
  const kecamatanMap = useMemo(() => {
    const m = {};
    rawRows.forEach(r => { if (!m[r.kecamatan]) m[r.kecamatan] = []; m[r.kecamatan].push(r); });
    return m;
  }, [rawRows]);

  // ── Statistik global (4 kartu utama) ──
  const globalStats = useMemo(() => {
    if (!rawRows.length) return null;
    const kecamatanSet = new Set(rawRows.map(r => r.kecamatan).filter(Boolean));
    const sudahDidata = rawRows.filter(r => r.hasilIdentifikasi.trim().toLowerCase() === "sudah di data").length;
    const belumDidata = rawRows.length - sudahDidata;
    return {
      totalKecamatan: kecamatanSet.size,
      totalBumdes: rawRows.length,
      sudahDidata,
      belumDidata,
    };
  }, [rawRows]);
  ...
  // Angka utama di kartu = jumlah yang BELUM didata (lebih butuh perhatian),
  // sub-info di bawahnya = jumlah yang sudah didata.
  const kecamatanList = useMemo(() => {
    const namaList = new Set([...KECAMATAN_ORDER, ...Object.keys(kecamatanMap)]);
    return [...namaList]
      .map(nama => {
        const list = kecamatanMap[nama] || [];
        const countSudah = list.filter(r => r.hasilIdentifikasi.trim().toLowerCase() === "sudah di data").length;
  ...
  const selectedRows = useMemo(() => {
    ...
    return [...(kecamatanMap[selectedKec] || [])].sort((a, b) => a.namaBumDesa.localeCompare(b.namaBumDesa));
  }, [selectedKec, kecamatanMap]);

  // ── 2 kategori status (Sudah / Belum Didata) untuk kecamatan terpilih ──
  const statusCounts = useMemo(() => {
    const sudah = selectedRows.filter(r => r.hasilIdentifikasi.trim().toLowerCase() === "sudah di data").length;
  ...
  const filteredRows = useMemo(() => {
    ...
    if (statusFilter === "sudah") {
      result = result.filter(r => r.hasilIdentifikasi.trim().toLowerCase() === "sudah di data");
    } else if (statusFilter === "belum") {
      result = result.filter(r => r.hasilIdentifikasi.trim().toLowerCase() !== "sudah di data");
    }

    if (searchBumdes.trim()) {
      const q = searchBumdes.toLowerCase();
      result = result.filter(r =>
        (r.namaBumDesa || "").toLowerCase().includes(q) ||
        (r.desa || "").toLowerCase().includes(q)
      );
    }
    return result;
  }, [selectedRows, statusFilter, searchBumdes]);

  // Modal DetailBumdes menutup dirinya sendiri (lewat onClose) setelah
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans overflow-x-hidden">
      ...
      {modalRow && (
        <DetailBumdes row={modalRow} onClose={() => setModalRow(null)} onSaved={handleSaved} />
      )}
      ...
      <header className="relative overflow-hidden" style={{ background: "linear-gradient(135deg,#fb923c 0%,#f97316 45%,#ea580c 100%)" }}>
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
          <div>
            <h1 className="text-white text-xl sm:text-2xl md:text-3xl font-black leading-tight">Monitoring Pendataan Badan Usaha Milik Desa</h1>
            <p className="text-orange-100 mt-1 text-sm sm:text-base">Daftar Badan Usaha Milik Desa</p>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto w-full px-4 sm:px-6 md:px-6 py-6 sm:py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-400 text-sm">Mengambil data Badan Usaha Milik Desa dari spreadsheet…</p>
          </div>
        )}
        ...
        {!loading && !error && globalStats && (
          <>
            {/* ── Stat Cards global: 4 kartu ── */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-3.5 mb-6 sm:mb-7">
              <StatCard
                label="Jumlah Kecamatan"
                value={globalStats.totalKecamatan}
                sub="kecamatan terdata"
                icon="📍"
                accentColor="#ea580c"
              />
              <StatCard
                label="Jumlah Badan Usaha Milik Desa"
                value={globalStats.totalBumdes}
              ...
              <StatCard
                label="Sudah Didata"
                value={globalStats.sudahDidata}
                sub={`${globalStats.belumDidata} belum didata`}
                icon="✅"
                accentColor="#0f766e"
              />
              <StatCard
                label="Belum Didata"
                value={globalStats.belumDidata}
                sub={`${globalStats.sudahDidata} sudah didata`}
                icon="⏳"
                accentColor="#e11d48"
              />
            </div>
              ...
              <div ref={detailRef} className="w-full min-w-0 lg:w-[45%]">
                {!selectedKec ? (
                  <div className="lg:sticky lg:top-6 rounded-2xl border-2 border-dashed border-gray-200 bg-white flex flex-col items-center justify-center py-16 sm:py-20 text-center px-6 sm:px-8">
                    ...
                    <p className="text-gray-400 text-sm mt-1">Klik kartu kecamatan untuk melihat daftar Badan Usaha Milik Desa di wilayah tersebut.</p>
                  </div>
                ) : (
                  <div className="lg:sticky lg:top-6 w-full min-w-0 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="px-5 sm:px-6 py-4 sm:py-5" style={{ background: "linear-gradient(135deg,#F5A623 0%,#e8820a 100%)" }}>
                      ...
                      <div className="mt-2 flex items-center justify-between text-white text-sm">
                        <span className="opacity-80">Total Badan Usaha Milik Desa di kecamatan ini</span>
                    ...
                    <div ref={tableRef} className="px-4 sm:px-6 max-h-[460px] sm:max-h-[520px] overflow-y-auto">
                      <div className="sticky top-0 bg-white pt-2 pb-1 z-10">
                        <div className="relative mb-2">
                          ...
                          <input
                            type="text" placeholder="Cari nama Badan Usaha Milik Desa / desa" value={searchBumdes}
                            onChange={e => { setSearchBumdes(e.target.value); if (tableRef.current) tableRef.current.scrollTop = 0; }}
                          ...
                          />
                          {searchBumdes && (
                            <button onClick={() => setSearchBumdes("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500">
                      ...
                      {filteredRows.length === 0 ? (
                        <div className="text-center py-10">
                          ...
                          <p className="text-gray-400 text-sm">
                            {searchBumdes
                              ? `Tidak ada hasil untuk "${searchBumdes}"`
                              : statusFilter === "sudah"
                              ? "Tidak ada data yang sudah didata."
                              : statusFilter === "belum"
                              ? "Tidak ada data yang belum didata."
                              : "Belum ada data Badan Usaha Milik Desa."}
                          </p>
                        </div>
                      ) : (
                    ...
                    <div className="px-4 sm:px-6 py-3.5 bg-gray-50 border-t border-gray-100">
                      <div className="flex gap-2 text-xs text-gray-400 flex-wrap items-center">
                        <span className="font-semibold text-gray-600">
                          {filteredRows.length}{(searchBumdes || statusFilter) ? ` / ${selectedRows.length}` : ""} Badan Usaha Milik Desa
                        </span>
                        {statusFilter ? (
                          <span className={`font-medium ${statusFilter === "sudah" ? "text-emerald-600" : "text-rose-500"}`}>
                            · {statusFilter === "sudah" ? "Sudah Didata" : "Belum Didata"}
                          </span>
                        ) : (
                          <>
                            <span>·</span>
                            <span className="text-gray-400 font-medium">
                              {filteredRows.filter(r => r.hasilIdentifikasi.trim().toLowerCase() !== "sudah di data").length} belum didata
                            </span>
                          </>
                        )}
                      </div>
