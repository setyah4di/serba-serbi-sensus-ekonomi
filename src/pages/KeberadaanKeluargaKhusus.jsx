...
// Spreadsheet: Daftar Keluarga Khusus (pesantren, panti asuhan dkk)
// https://docs.google.com/spreadsheets/d/1C5de48P3U_nvudg4IKx-W5gRUf7bv_0k-bQ2BKUyqLk/edit?gid=379545987
const SPREADSHEET_ID = "1C5de48P3U_nvudg4IKx-W5gRUf7bv_0k-bQ2BKUyqLk";
const GID_PESANTREN = "379545987";
const CSV_PESANTREN = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/export?format=csv&gid=${GID_PESANTREN}`;

// URL Web App Apps Script yang menangani update kolom E, H, I
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzXwDGNJ9JOXsRjhI-Oc77jUcCKEXRIsqyVHYwozXIs79laWDA1mvHGkmblAYfDPZNdzQ/exec";
...
// Opsi hasil identifikasi (sesuai dropdown kolom H di spreadsheet)
...
// ── Baris pesantren ──
function PesantrenRow({ row, rank, onDetail }) {
  return (
    <div className="py-3 border-b border-gray-50 last:border-0">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 truncate mt-0.5">{row.alamat || "Alamat belum diketahui"}</p>
          <div className="mt-1.5 space-y-1">
            ...
            {row.usaha && (
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[10px] text-gray-400 font-medium w-[105px] flex-shrink-0">Usaha</span>
                <span className="text-[10px] text-gray-500">{row.usaha}</span>
              </div>
            )}
          </div>
        </div>
...
function DetailPesantren({ row, onClose, onSaved }) {
  ...
  const [usaha, setUsaha] = useState(row.usaha || "");
        ...
        <div className="px-5 py-4" style={{ background: "linear-gradient(135deg,#F5A623 0%,#e8820a 100%)" }}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-orange-100 text-[10px] font-semibold uppercase tracking-widest mb-0.5">Konfirmasi Data Pesantren</p>
        ...
        <div className="px-5 py-4 space-y-3.5 max-h-[70vh] overflow-y-auto">
          <table className="w-full text-xs border border-gray-100 rounded-xl overflow-hidden">
            <tbody>
              ...
              <tr className="border-b border-gray-100">
                <td className="bg-gray-50 px-3 py-2 font-semibold text-gray-500 align-top">Jenis</td>
                <td className="px-3 py-2 text-gray-700">{row.jenis || "-"}</td>
              </tr>
              <tr>
                <td className="bg-gray-50 px-3 py-2 font-semibold text-gray-500 align-top">Alamat</td>
                <td className="px-3 py-2 text-gray-700">{row.alamat || "-"}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div>
            ...
            <input
              ...
              disabled={isBusy}
              placeholder="Contoh: kantin, koperasi, sekolah…"
...
// KOMPONEN UTAMA
// ══════════════════════════════════════════════════════════════════════════════
export default function KeberadaanKeluargaKhusus() {
  ...
  // ── Fetch CSV ──
  useEffect(() => {
    fetch(`${CSV_PESANTREN}&_cb=${Date.now()}`)
      .then(r => { if (!r.ok) throw new Error("Gagal mengambil data pesantren."); return r.text(); })
      .then(text => {
        ...
        parsed.slice(1).forEach((cols, idx) => {
          ...
          data.push({
            ...
            hasilIdentifikasi: (cols[7] || "").trim(),  // H
            usaha: (cols[8] || "").trim(),              // I
          });
        });
  ...
  const selectedRows = useMemo(() => {
    ...
    return [...(kecamatanMap[selectedKec] || [])].sort((a, b) => a.nama.localeCompare(b.nama));
  ...
  const filteredRows = useMemo(() => {
    ...
    if (searchPesantren.trim()) {
      const q = searchPesantren.toLowerCase();
    ...
    return result;
  }, [selectedRows, statusFilter, searchPesantren]);

  // Modal DetailPesantren menutup dirinya sendiri (lewat onClose) setelah
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans overflow-x-hidden">
      ...
      {modalRow && (
        <DetailPesantren row={modalRow} onClose={() => setModalRow(null)} onSaved={handleSaved} />
      )}
      ...
      <header className="relative overflow-hidden" style={{ background: "linear-gradient(135deg,#fb923c 0%,#f97316 45%,#ea580c 100%)" }}>
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
          <div>
            <h1 className="text-white text-xl sm:text-2xl md:text-3xl font-black leading-tight">Monitoring Pendataan Keluarga Khusus</h1>
            <p className="text-orange-100 mt-1 text-sm sm:text-base">Daftar Keluarga Khusus (Pesantren, Panti Asuhan, dkk)</p>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto w-full px-4 sm:px-6 md:px-6 py-6 sm:py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-400 text-sm">Mengambil data pesantren dari spreadsheet…</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-3.5 mb-6 sm:mb-7">
              ...
              <StatCard
                ...
                value={globalStats.totalPesantren}
              ...
              <div ref={detailRef} className="w-full min-w-0 lg:w-[45%]">
                {!selectedKec ? (
                  <div className="lg:sticky lg:top-6 rounded-2xl border-2 border-dashed border-gray-200 bg-white flex flex-col items-center justify-center py-16 sm:py-20 text-center px-6 sm:px-8">
                    ...
                    <p className="text-gray-400 text-sm mt-1">Klik kartu kecamatan untuk melihat daftar pesantren di wilayah tersebut.</p>
                  </div>
                ) : (
                  <div className="lg:sticky lg:top-6 w-full min-w-0 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="px-5 sm:px-6 py-4 sm:py-5" style={{ background: "linear-gradient(135deg,#F5A623 0%,#e8820a 100%)" }}>
                      ...
                      <div className="mt-2 flex items-center justify-between text-white text-sm">
                        <span className="opacity-80">Total pesantren di kecamatan ini</span>
                    ...
                    <div ref={tableRef} className="px-4 sm:px-6 max-h-[460px] sm:max-h-[520px] overflow-y-auto">
                      <div className="sticky top-0 bg-white pt-2 pb-1 z-10">
                        <div className="relative mb-2">
                          ...
                          <input
                            type="text" placeholder="Cari nama / alamat pesantren" value={searchPesantren}
                            onChange={e => { setSearchPesantren(e.target.value); if (tableRef.current) tableRef.current.scrollTop = 0; }}
                          ...
                          />
                          {searchPesantren && (
                            <button onClick={() => setSearchPesantren("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500">
                      ...
                      {filteredRows.length === 0 ? (
                        <div className="text-center py-10">
                          ...
                          <p className="text-gray-400 text-sm">
                            {searchPesantren
                              ? `Tidak ada hasil untuk "${searchPesantren}"`
                              : statusFilter === "sudah"
                    ...
                    <div className="px-4 sm:px-6 py-3.5 bg-gray-50 border-t border-gray-100">
                      <div className="flex gap-2 text-xs text-gray-400 flex-wrap items-center">
                        <span className="font-semibold text-gray-600">
                          {filteredRows.length}{(searchPesantren || statusFilter) ? ` / ${selectedRows.length}` : ""} Pesantren
                        </span>
                        {statusFilter ? (
